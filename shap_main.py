...
if '__main__' == __name__:
    ...
    # Split train
    X = small_train.iloc[:, :TARGET_COLUMN]
    y = small_train.iloc[:, TARGET_COLUMN]
    ...
    # Models with params - we'll use it later
    # models_params_pairs = [
    #     (baseline_model, baseline_model_params),
    #     (nanthresh_model, nanthresh_model_params),
    #     (xgb_model, xgb_model_params)
    # ]
    ...
    data_for_prediction = X_test[rows_to_show]

    explainer = shap.TreeExplainer(cv.best_estimator_['model'])
    for row in range(NUM_EXPLAINED_ROWS):
        data_for_prediction = X_test[rows_to_show[row]]
        shap_values = explainer.shap_values(data_for_prediction)
        shap.summary_plot(shap_values, data_for_prediction, plot_type="bar",show=False)
        shap_summary_plot_path = os.path.join(PLOTS_DIR + '/' + model_name + '_'
                                              + datetime.now().strftime("%d-%m-%H-%M-%S")
                                              + '_Best-%.3f' % result_test
                                              + 'shap_summ_plot_row-%d' % row)
        plt.savefig(shap_summary_plot_path)

        row_path = os.path.join(PLOTS_DIR + '/' + model_name + '_'
                                + datetime.now().strftime("%d-%m-%H-%M-%S")
                                + '_Best-%.3f' % result_test
                                + 'row-%d' % row)
        data_for_prediction.to_csv(row_path, index=False, header=True)
